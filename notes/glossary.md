...
- 语法：`x := expr`，只能在函数体内使用；等价于“声明 + 赋值 + 类型推导”。
- 规则：同一作用域里，`:=` 要求左侧**至少一个新变量名**；否则编译报错。
- 易踩坑：在内层 block 里用 `:=` 写出同名变量会发生 shadowing（你以为更新外层，其实新建了内层）。
...
- 常见内容：用户名、密码、地址、库名、连接参数。
- MySQL 里常见的 `parseTime=true` 用来让 `DATETIME/TIMESTAMP` 正确扫描到 `time.Time`。
...
## Interface compliance assertion（编译期接口校验）
- 用法：`var _ SomeInterface = (*T)(nil)`，用于在编译期强制检查 `*T` 是否实现接口。
- 作用：重构/拆包后，如果漏实现接口，会在编译期立刻失败，而不是运行期才踩坑。
- 注意：这是“带类型的 nil”做静态赋值检查，不会在运行期创建对象。

## Conversion（显式类型转换）
- Go 不做隐式数值类型转换；不同数值类型相加/比较需要显式转换（例如 `int64(x)`）。
- 转换可能导致溢出/截断（`float64→int` 截断小数；`int64→int` 可能溢出）。

## Interpreted string literal（解释型字符串字面量）
- 语法：`"..."`，支持转义序列（`\n/\t/\\/\uXXXX` 等）。
- 场景：需要精确控制换行、制表、引号、反斜杠时使用。
- 工程建议：日志/调试打印用户输入时常配合 `%q`，避免控制字符破坏日志结构。

## Raw string literal（原生字符串字面量）
- 语法：`` `...` ``，内容原样保留，可多行；不处理转义序列。
- 场景：正则/SQL/JSON/模板等包含大量反斜杠或多行文本时更易读。
- 限制：不能包含反引号 `` ` `` 本身。
