...
## 原生字符串不处理转义
- `` `\n` `` 里就是反斜杠 + n，不会变成换行。
- 避免：需要换行/制表等转义时用解释型字符串 `"..."`。

## `Trim` 不是“移除子串”
- `strings.Trim(s, cutset)` 是按“字符集合”修剪，不是移除某个固定前缀/后缀。
- 避免：删前缀/后缀用 `TrimPrefix/TrimSuffix`。

## `Split` 对空字符串的结果不是空切片
- `strings.Split("", ",")` 会返回 `[]string{""}`（长度 1）。
- 避免：空输入先 `if s == "" { return nil }` 或用 `FieldsFunc` 等更合适的策略。

## 循环里用 `+=` 拼接可能退化
- 多次拼接会产生大量临时字符串与重复分配，规模上去后可能退化为 O(n^2)。
- 避免：循环拼接用 `strings.Builder`，必要时先 `Grow`。
...
## 重复实现 HTTP helper 导致行为漂移
- 同一个项目里出现多个 `writeJSON/writeError/parsePageSize` 版本，很容易某个接口漏校验或响应结构不同。
- 避免：抽到统一 helper 包，所有 handler 只调用同一份实现。
